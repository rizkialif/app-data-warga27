...
export default function KetuaRtPage() {
  ...
  const canCreate = hasPermission('action:create:ketua-rt');
  const canUpdate = hasPermission('action:update:ketua-rt');
  const canDelete = hasPermission('action:delete:ketua-rt');
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <Title level={4} style={{ margin: 0 }}>Data Ketua RT</Title>
          <Text type="secondary">Kelola data nama-nama Ketua Rukun Tetangga (RT)</Text>
        </div>
        <TableActions
